...
def filter_data(txt_input, txt_output):
    ...
    with open(txt_input) as ifile:
        for line in ifile:
            ...
            if click_flag == '1' and label == '0':
                continue
            if click_flag == '0':
                if random.random() > 0.2:
                    continue
            ofile.write('\t'.join([uid, docid, time, click_flag, label]))
            ofile.write('\n')
    ofile.close()
...
def read_offline():
    data_train = pd.read_csv('train_set_sample', sep='\t', header=None,\
            names=['cid','did', 'time', 'label1', 'label2'])
    doc_info = pd.read_csv('train_nid_data',\
            sep='\t',\
            header=None,\
...
def split_train_test():
    """
    """
    df = pd.read_csv('merge.csv', sep='\t', na_values='\\N')
    df = df.fillna(' ')
...
if __name__ == '__main__':
    ...
    if fun in ['filter_data', 'parse_data', 'filter_data_parse_time', 'filter_data_keep_time']:
        ...
        pass
    split_train_test()
    filter_data('../train/train_set', 'train_set_sample')
    parse_data('../train/train_nid_data', 'train_nid_data')
    read_offline()
    """
    split_train_test()

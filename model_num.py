...
def build_model(max_ele_len):
    ...
    # pic_num
    pic_num_input = Input(shape=(1, ), dtype='float32')

    # content_len
    content_input = Input(shape=(1, ), dtype='float32')
    ...
    act2 = a(concat2)
    den1 = Dense(units=60, activation='relu', kernel_regularizer=regularizers.l2(0.01))(act2)
    den2 = Dense(units=12, activation='relu', kernel_regularizer=regularizers.l2(0.01))(den1)
    concat3 = concatenate([den2, pic_num_input, content_input])
    den3 = Dense(units=5, activation='softmax', kernel_regularizer=regularizers.l2(0.01))(concat3)

    model = Model(inputs=[uid_input, cate_input, tag1_input, tag2_input, key_word_input,\
            author_input, pic_num_input, content_input], outputs=den3)
...
if __name__ == '__main__':
    ...
    used_fea = ['cid','cate','tag1','tag2', 'key_word', 'author', 'pic_num', 'content_len']
    ...
    for ele in used_fea:
        ...
        elif ele in ['pic_num', 'content_len']:
            values = all_data[ele].values
            values = values.reshape(values.shape[0], -1)
            scale = preprocessing.StandardScaler().fit(values)
            array_id = scale.transform(values)
            data_train.append(array_id[:train_index_split])
            data_eval.append(array_id[train_num:])
            print(ele, -1)
            max_ele_len.append(-1)
            word2id_lst.append(scale)
        else:
    ...
    for i, fea in enumerate(used_fea):
        ...
        elif fea in ['pic_num', 'content_len']:
            values = raw_test_data[fea].values
            values = values.reshape(values.shape[0], -1)
            scale = word2id_lst[i]
            array_id = scale.transform(values)
            data_predict.append(array_id)
        else:

...
class convertor(object):
    ...
    def write_label(self, df, name):
        labels2 = df['label2'].astype(int).values - 1
        labels2 = to_categorical(np.asarray(labels2))
        np.savetxt('train/feature/%s_label2.txt' % name, labels2, fmt="%d")
        labels1 = df['label1'].astype(int).values
        labels1 = labels1.reshape([labels1.shape[0], 1])
        np.savetxt('train/feature/%s_label1.txt' % name, labels1, fmt="%d")
...
class filter_unclick(object):
    def __init__(self, used_fea, label1):
        self.used_fea = used_fea
        self.label1 = label1
        self.run('train')
    def run(self, _type):
        # train/feature/test_label1.txt
        prefix = 'train/feature/'
        txt_label1 = "%s%s_%s.txt" % (prefix, _type, self.label1)
        keep_index = [i for i, x in enumerate(open(txt_label1, 'r')) if int(x)==1]
        for fea in self.used_fea:
            fname =  "%s%s_%s.txt" % (prefix, _type, fea)
            print(fname)
            data = np.loadtxt(fname, dtype=np.int32, ndmin=2)
            keep_data= data[keep_index]
            np.savetxt('train/buffer/%s_%s.txt' % (_type, fea), keep_data, fmt="%d")
...
if __name__ == '__main__':
    ...
    conv = convertor(used_fea, extracts, train_txt, test_txt, online_txt, True)
    conv.run()
    """
    used_fea = ['cid','cate','tag1','tag2', 'author', 'key_word', 'title', 'label2']
    fu = filter_unclick(used_fea, 'label1')
